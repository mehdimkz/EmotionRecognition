...
load(Name);
      %Tr=4 has problem with relief feature section for subkect 1,viedo 32
      %va 36..If condition is required
Tr=1; %classification based on Valence=1,Arousal=2,Liking=4......Must be set before running
...
nInput=78;  %numbers of total features 
%Data=featmatlabels;
z=32; %number of subjcts
%***Implementation of One viedo-out- SVM Cross validation
f=7; %determine number of feature ranking method
...
d=8;%number of features of  each feature set
...
if i==1
 ...
 elseif i==2
  ...
  % elseif i==8  %If Tr=4(liking),then releif has problem for subject1,viedo 32,36 we use if.
  % methodFS='relief';
...
%{
 ...
 if i==1
  ...
  methodFS='relief';
  %if (s~=3 || j~=1 && j~=2) %If Tr=4(liking),-then for solving subject 1 viedos 32 and 36 problem with relief this if is necessery.
  if (s~=3 && s~=12 && s~=13 && s~=21 && s~=26 && s~=30)
 [selectedFeatures]=feast(methodFS,nInput,TrnData(:,1:nInput), TrnData(:,nInput+1)); 
  end
...
ANN_Majority_lables_rankingalgo(:,i)=mode(ANN_prd_labels(:,n*i-8:n*i),2);
labels(:,Tr)== ANN_Majority_lables_rankingalgo(:,i);
