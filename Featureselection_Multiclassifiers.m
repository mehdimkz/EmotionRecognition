...
tic; 
f=4; %***numbers of feature selection algorithem-must be set
...
[TrnData] = removerows(Data(:,:),'ind',[j]);
TstData=Data(j,:)
%**for showing the program progress(optional)
 step=step+1;
...
for n=1:x  %x numbers of feature sets
    ...
    LDA_FeatureSets_Accuracy(i,n)=mean(ans)*100; 
    labels(:,1)==ANN_predicted_labels(:,10*i-10+n);
    ANN_FeatureSets_Accuracy(i,n)=mean(ans)*100;
    labels(:,1)==CART_predicted_labels(:,10*i-10+n);
    CART_FeatureSets_Accuracy(i,n)=mean(ans)*100;
    
end
Majority_LDA_lables_rankingalgo(:,i)=mode(LDA_predicted_labels(:,n*i-9:n*i),2);
labels(:,1)== Majority_LDA_lables_rankingalgo(:,i);
LDA_Featureselection_Accuracy(1,i)=mean(ans)*100;

Majority_ANN_lables_rankingalgo(:,i)=mode(ANN_predicted_labels(:,n*i-9:n*i),2);
labels(:,1)== Majority_ANN_lables_rankingalgo(:,i);
ANN_Featureselection_Accuracy(1,i)=mean(ans)*100;

Majority_CART_lables_rankingalgo(:,i)=mode(CART_predicted_labels(:,n*i-9:n*i),2);
labels(:,1)== Majority_CART_lables_rankingalgo(:,i);
CART_Featureselection_Accuracy(1,i)=mean(ans)*100;
end
%calculating the majority vote 
 ...
 FinalPredicatedLabelsCART = mode(CART_predicted_labels,2);
  
labels(:,1)== FinalPredicatedLabelsLDA;
LDA_FinalAccuracy=mean(ans)*100;

labels(:,1)== FinalPredicatedLabelsANN;
ANN_FinalAccuracy=mean(ans)*100;

labels(:,1)== FinalPredicatedLabelsCART;
CART_FinalAccuracy=mean(ans)*100;

%**Calculating Multiclassifier lables and accuracy
Multiclassifires_merged_labels=cat(2,LDA_predicted_labels,ANN_predicted_labels,CART_predicted_labels);
FinalMultiClassifires_Labels= mode(Multiclassifires_merged_labels,2);
labels(:,1)== FinalMultiClassifires_Labels;
